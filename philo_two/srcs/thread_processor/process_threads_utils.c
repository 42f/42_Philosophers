...
/*   Updated: 2020/12/18 09:59:44 by bvalette         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		init_sem(t_data *data)
{
	...
	data->sem_forks_heap = safe_sem_open(SEM_NAME_FORKS_HEAP,
														data->param[NB_PHILO]);
	if (data->sem_nb_philo_done == SEM_FAILED || data->sem_stdout == SEM_FAILED
	|| data->sem_death_report == SEM_FAILED
	|| data->sem_forks_heap == SEM_FAILED
	|| data->sem_race_starter == SEM_FAILED)
		return (cleanup_routine(CODE_ERR_SEM));
	else
		return (SUCCESS);
}
