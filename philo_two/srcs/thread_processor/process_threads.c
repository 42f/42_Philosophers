...
/*   Updated: 2020/12/18 10:07:44 by bvalette         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				process_philo(t_data *data)
{
	...
	if (pthread_create(&th_clock, NULL, clock_routine, data) != 0)
		return (cleanup_routine(CODE_ERR_PTHREAD));
	if (thread_creation(data, th_philo,
									philo_state_machine, philo_id) != SUCCESS)
		return (FAILURE);
	if (thread_creation(data, th_monitor, philo_monitor, philo_id) != SUCCESS)
		return (FAILURE);
